-- GHQ-12 and GHQ-28 questionnaire content, promoted into the migration set.
--
-- ─────────────────────────────────────────────────────────────────────
-- WHY THIS IS NOW A MIGRATION.
--
-- Held until now in `supabase/instrument-content/00040_...sql`, outside the
-- migration set so that `db push` could not ship it. That file opens with a
-- guard refusing a non-local host, which directs the reader to promote the
-- content to a migration instead. This is that promotion, so the guard is not
-- reproduced: it exists to stop the HELD copy reaching production by accident,
-- not to stop an authorised release. The held file is left byte-for-byte
-- unchanged, so its provenance and guard survive as the audit trail.
--
-- THE AUTHORISATION, STATED PRECISELY.
--
-- The Occupational Health physician directing this engagement confirmed on
-- 2026-08-29 that the GHQ content already supplied for the existing Microsoft
-- Forms process is the content to use, and authorised it for INTERNAL USER
-- TESTING.
--
-- What that authorisation is NOT, stated so nobody later reads more into it:
--
--   · It is NOT confirmation that the attribution wording required by
--     GL Assessment has been contractually agreed. It has not been. The
--     `licence_note` written by this content still records that requirement,
--     and `data/wellbeing-instruments.ts` still carries `attribution: null`
--     rather than an invented string.
--   · It is NOT approval for external or commercial distribution. That
--     remains a separate compliance gate.
--
-- No wording here was reconstructed, paraphrased or sourced from the internet.
-- It is the content already supplied for this engagement.
--
-- GHQ-28 SECTION D.
--
-- Section D asks directly about not wanting to live. GHQ-28 becomes servable
-- only because the participant-facing support pathway is implemented and
-- carries an interim internal-test approval, recorded in
-- `data/ghq28-support-content.ts` as `GHQ28_SUPPORT_APPROVAL_STATE =
-- 'interim_internal_test'`. Final clinical-governance sign-off is still
-- required before GHQ-28 is offered beyond this internal test, and the
-- constant exists so that requirement cannot be lost in a comment.
--
